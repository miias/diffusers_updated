...
class DependencyTester(unittest.TestCase):
    def test_diffusers_import(self):
        try:
            import diffusers_udated  # noqa: F401
        except ImportError:
            assert False

    def test_backend_registration(self):
        import diffusers_udated
        from diffusers_udated.dependency_versions_table import deps

        all_classes = inspect.getmembers(diffusers_udated, inspect.isclass)
    ...
    def test_pipeline_imports(self):
        import diffusers_udated
        import diffusers_udated.pipelines

        all_classes = inspect.getmembers(diffusers_udated, inspect.isclass)
        for cls_name, cls_module in all_classes:
            if hasattr(diffusers_udated.pipelines, cls_name):
